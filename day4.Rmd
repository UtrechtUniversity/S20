# Day 4: Traits, timing, and measurement error {-}
...
## Exercise 1: The mean-structure{-}

#### Exercise 1A{-}
Specify a model in which you investigate whether the group *means* can be constrained across time. Does this model fit? Tip: Allow all variables to covary with each other (i.e. specify a model without constraints on the covariance structure) such that we are estimating means, and not intercepts.
...
The input for this models can be found in `exercise2A.inp`.
...
The input for this models can be found in `exercise2B.inp`.
