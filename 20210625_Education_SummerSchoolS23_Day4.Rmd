...
<details>
  <summary><b>Click to show answers</b></summary>
See the Mplus input file `ExerciseB.inp`. 
...
See the Mplus input file `ExerciseE.inp` for how to constrain the measurement error variances over time. With constrained measurment error variances, we estimate 3 parameters less. So, we estimate $11 - 3 = 8$ and therefore have $10 - 8 = 2$ *df*. 
...
In your output, look at the reported (estimated) residual variances. We find that the residual variance of `ETA4` is estimated to be negative. This is a Heywood case and it is causing the warning to appear. Note however, that this time it is significant.
...
<details>
  <summary><b>Click to show answers</b></summary>
Yes, they are nested: this model is a special case of the previous model, as it is based on having `eta3` and `eta4` from the previous model now being a single latent variable. That is, the residual variance of eta 4 could be fixed to zero: this gives us 1 *df* for the difference.
`r if(knitr::is_html_output()){"\\details"}`
...
<details>
  <summary><b>Click to show answers</b></summary>
See the Mplus input file `Excercise1j.inp` for the model specification in Mplus. Apart from the $\chi^{2}$-test of model fit, the model fits well: 
...
<details>
  <summary><b>Click to show answers</b></summary>
Comparing both models using the $\Delta \chi^{2}$-test gives us $27.37 – 13.29 = 14.0$ with 1 *df* such that $p < .001$. This implies that imposing the restriction is not tenable. 
...
<details>
  <summary><b>Click to show answers</b></summary>
See Exercise1m.inp for the Mplus specification of this model. The `Y3 WITH Y4` parameter is an additional covariance between the residuals of `y3` and `y4` (so not between `y3` and `y4` themselves). Model fit is quite good (except for the $\chi^{2}$-test of model fit): 
...
#### Exercise F{-}
Specify the CLPM and run this model. Compare it to the previous two model. How are these models related?
...
iscuss how the model results differ.
